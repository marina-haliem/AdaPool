...
class Vehicle(object):
    ...
    def step(self, timestep):
        ...
        if self.__behavior.available and self.state.current_capacity == 0:
           # print("IDLE!")
            self.state.idle_duration += timestep
        else:
            self.state.idle_duration = 0

        self.duration[self.state.status] += timestep
    ...
    def pickup(self, customer):
        ...
        if len(self.current_plan) == 0:
            ...
            pick_drop = self.pickup_flags[0]
            # self.state.accept_new_request = False
            ...
            if triptime == 0.0:
                ...
                self.set_route(route, speed)
                self.set_destination(route[-1], triptime)
                # print(self.get_id(), "Dest: ", self.get_destination(), triptime)
                self.change_to_occupied()
    ...
    def dropoff(self, customer):
        ...
        self.accepted_customers.remove(customer.get_id())
        customer.get_off()
        ...
        if (self.state.current_capacity == 0) | (len(self.current_plan) == 0):
            ...
            self.accepted_customers = []
            self.onboard_customers = []
            self.state.tmp_capacity = 0
            self.state.init_match_count = 0
            self.ordered_pickups_dropoffs_ids = []  # Queue of IDs to determine next customer to be picked up or dropped off
            self.pickup_flags = []  # Queue of Flags to determine whether next stop is pickup or dropof
            self.current_plan = []  # Queue of locations determining th current planned trip
            self.current_plan_routes = []  # Queue of routes corresponding to each location on the current trip
