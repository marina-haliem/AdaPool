...
class Central_Agent(object):
    ...
    def getChunks_client(self, command, n):
        arr = []
        ...
        for index in range(len(command["customer_id"])):
            ...
            if customer is None:
                ...
                continue
            c["customer_id"] = command["customer_id"][index]
            c["init_price"] = command["init_price"][index]
            arr.append(c)

        # print(arr)
        out = self.getChunks(arr,n)
        # print(out)
    ...
    def get_match_commands(self, current_time, vehicles, requests):
        ...
        if len(requests) > 0:
            ...
            init_match_time = 0
            greedy_match_time = 0
            fst_consolidate_time = 0
            snd_consolidate_time = 0
            # while (n):
                # if FLAGS.enable_pooling:
            # print("R: ", len(requests), "V: ", len(vehicles))
            idle_vehicles = vehicles[
                ...
                #| ((vehicles.status == status_codes.V_ASSIGNED) & vehicles.accept_new_request)
            ]
            # I = vehicles[
            #     (((vehicles.status == status_codes.V_IDLE) |
            #       (vehicles.status == status_codes.V_CRUISING)) &
            #      (vehicles.idle_duration > 0)) ]
            # print("Real Idle: ", len(I))
            # print("Idle: ", len(idle_vehicles))
            s1 = t.time()
            matching_commands = self.matching_policy.match_RS(current_time, vehicles, requests)
            e2 = t.time()
            init_match_time += (e2-s1)
            ...
            e6 = t.time()
            # print("1st price and consolidate: ", e6-s6)
            fst_consolidate_time += (e6-s6)
            ...
            # print("len(final_commands) = ", len(final_commands))
            ...
            # if num_unmatched/total_reqs < 0.1:
                # if num_unmatched == 0:
                # break
            ...
            e5 = t.time()
            # print("Consolidate Time: ", e5-s5)
            snd_consolidate_time += (e5-s5)
            ...
            for c in finalize_commands:
                ...
                # print("Vid: ", vid, v.state.status, v.state.accept_new_request)
                if len(v.current_plan) != len(v.current_plan_routes) != len( \
                        v.ordered_pickups_dropoffs_ids) != len(v.pickup_flags):
                    print("1.ERROR!")
                # vehicle.change_to_assigned()
                v.state.init_match_count = 0
                if len(c["customer_id"]) > 0:
                    # v.state.status = status_codes.V_ASSIGNED
                    total_vehicles.loc[vid, "status"] = status_codes.V_ASSIGNED
                    for id in range(len(c["customer_id"])):
                        cid = c["customer_id"][id]
                        cust = CustomerRepository.get(cid)
                        if cust is None:
                            print("Invalid Customer")
                        if cid in matched_req:
                            # vid = c["vehicle_id"]
                            # v = VehicleRepository.get(vid)
                            total_matched -= 1
                            # print("remove: ", cid)
                            # print(v.accepted_customers)
                            # print(v.ordered_pickups_dropoffs_ids)
                            # print(v.pickup_flags)
                            # print(v.current_plan)
                            # print(len(v.current_plan_routes))
                            v.accepted_customers.remove(cid)
                            pick_drop = np.array(v.ordered_pickups_dropoffs_ids)
                            ii = np.array(np.where(pick_drop == cid))[0]
                            # print(ii)
                            # ii = [list(x) for x, in ii]
                            # print(list(ii))
                            v.pickup_flags = np.delete(np.array(v.pickup_flags), ii).tolist()
                            # v.current_plan = np.delete(np.array(v.current_plan), ii).tolist()

                            occ = 0
                            if len(v.ordered_pickups_dropoffs_ids) != len(v.current_plan):
                                print("NEVER!!")
                                occ = 1
                            ...
                            for l in ii:
                                ...
                                if b > 0:
                                    l -= 1
                                v.current_plan_routes.pop(l)
                                v.current_plan.pop(l)
                                b += 1
                            v.ordered_pickups_dropoffs_ids = np.delete(np.array(v.ordered_pickups_dropoffs_ids), ii).tolist()
                            # print("After: ")
                            # print(v.accepted_customers)
                            # print(v.ordered_pickups_dropoffs_ids)
                            # print(v.pickup_flags)
            ...
            # print("Init Mathing Time: ", init_match_time)
            # print("Greedy Matching Time: ", greedy_match_time)
            # print("1st Cosolidate Time: ", fst_consolidate_time)
            # print("2nd Consolidate Time: ", snd_consolidate_time)
            # print("Matching Time: ", end-s, "Rate: ", (total_matched/total_reqs)*100, "%")
        else:
            return [], total_vehicles
        return finalize_commands, total_vehicles, total_matched
    ...
    def init_price(self, match_commands):
        ...
        for c in match_commands:
            ...
            if vehicle is None:
                ...
                continue
            vehicle.state.init_match_count = 0
            # print("Init: ", vehicle.state.status, vehicle.state.accept_new_request, vehicle.get_id())
    ...
    def generate_plan(self, vehicle, cust_list, new_customer_list, res_queue = None):
        out_dict = dict()
        # print("Plan: ", mp.current_process(), getpid(), new_customer_list)
        # if len(new_customer_list) == 1:
        c = []
        if type(new_customer_list) != list:
            # commands = list(commands)
            new_customer_list["customer_id"] = new_customer_list["customer_id"][0]
            c.append(new_customer_list)
            new_customer_list = c
        for new_cust in new_customer_list:
            cid = new_cust["customer_id"]
            # print(cid)
            c_price = new_cust["init_price"]
            new_customer = CustomerRepository.get(cid)
            if new_customer is None:
                print("Invalid Customer id")
            # for new_cust in new_customer_list:
            #     cid = new_cust["customer_id"]
            #     price = new_cust["price"]
            #     new_customer = VehicleRepository.get(cid)
            # global VehicleRepository
            ...
            out_dict[cid] = [insertion_cost, time_till_pickup, c_price, vehicle_plan, vehicle_flags, vehicle_ids,
                              vehicle_routes]

        if (res_queue != None):
            res_queue.put(out_dict)
        else:
            return out_dict
    ...
    def greedy_insertion(self, commands, result_queue = None):
        ...
        # global VehicleRepository
        # print("Here")
        # print("Greedy: ", mp.current_process(), getpid())
        s = t.time()
        ...
        for command in commands:
            ...
            if ((vehicle.state.status == status_codes.V_OCCUPIED) & vehicle.state.accept_new_request):
                    #| ((vehicle.state.status == status_codes.V_ASSIGNED) & vehicle.state.accept_new_request):
            ...
            while vehicle.state.tmp_capacity < vehicle.state.max_capacity:
                ...
                if len(command["customer_id"]) > 1:
                    # numCpu = mp.cpu_count()
                    # processMulti = int(len(command["customer_id"]) / 5)
                    # # if len(final_commands) > 30:
                    # if (processMulti >= 2):
                    #     processMulti = min(processMulti, numCpu)
                    chunks = self.getChunks_client(command, len(command["customer_id"]))

                    dict_cust_index = self.plan_wrapper(vehicle, vehicle_accepted_cust[vid], chunks)

                    # cid = command["customer_id"][index]
                else:
                    dict_cust_index = self.generate_plan(vehicle, vehicle_accepted_cust[vid], command)
                # [insertion_cost, waiting_time, Vplan, Vflags, Vids, Vroutes] = self.generate_plan(vehicle,
                    #                                                                       vehicle_accepted_cust[vid], customer)


                # print("Plan: ", dict_cust_index)
                if len(dict_cust_index) > 1:
                    for k,item in dict_cust_index.items():
                        insertion_cost_dict[k] = item[0]
                    ...
                    index = command["customer_id"].index(min_request_id)
                    command["customer_id"].remove(min_request_id)
                    command["duration"].remove(command["duration"][index])
                else:
                    min_request_id = list(dict_cust_index.keys())[0]
                    # index = 0
                    command["customer_id"] = []
                    command["duration"] = []

                cost, wait, price, Min_plan, Min_flags, Min_ids, Min_routes = np.copy(dict_cust_index[min_request_id])
    ...
    def plan_wrapper(self, vehicle, v_accepted_cust, chunks):
        processList = []
        res_queue = mp.Queue()

        for c in chunks:
            p = mp.Process(target=self.generate_plan, args=(vehicle, v_accepted_cust, c, res_queue,))
            processList.append(p)
            p.start()
