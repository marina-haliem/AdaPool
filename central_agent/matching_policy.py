...
class MatchingPolicy(object):
    ...
    def find_available_vehicles(self, vehicles):
        ...
        v_list = []
        for index, v in avaiable_vehicles.iterrows():
    ...
    def create_matching_dict(self, vehicle_id, customer_id, duration):
        match_dict = {}
    ...
    def match(self, current_time, vehicles, requests):
        ...
        for ridx, request_id in enumerate(requests.index):
            ...
            if d[ridx, vidx] < self.reject_distance:
                ...
                assignments.append(self.create_matching_dict(vehicle_id, request_id, duration))
...
class GreedyMatchingPolicy(MatchingPolicy):
    ...
    def assign_nearest_vehicle(self, request_ids, vehicle_ids, vehicle_idx, T):
        assignments = []
        for ri, rid in enumerate(request_ids):
            ...
            if vehicle.state.init_match_count < vehicle.state.max_init_matching:
                assignments.append((vid, rid, tt))
                vehicle.state.init_match_count += 1
    ...
    def assign_nearest_vehicle_RideShare(self, request_ids, all_target_ridx, requestAssigned, vehicle_ids,
                                         vehicle_idx, T, TRR, TDD):
        assignments = []
        # for ri, rid in enumerate(request_ids):
        ...
        for rid in request_ids:
            ...
            # print(rid, ": ", T[ri, vehicle_idx], len(T[ri, vehicle_idx]))
            # For this request ri, pick the min from all candidate vehicles
            vi = T[ri, vehicle_idx].argmin()

            tt = T[ri, vehicle_idx][vi]

            # print(T[ri, vehicle_idx][idx[:2])
    ...
    def assign_nearest_k_vehicle_RideShare(self, request_ids, all_target_ridx, requestAssigned, vehicle_ids,
                                         vehicle_idx, T, TRR, TDD):
        assignments = []
        # for ri, rid in enumerate(request_ids):
        # vi = T[ri,vehicle_idx].argmin()
        ...
        for rid in request_ids:
            ...
            if len(T[ri, vehicle_idx]) > 1:
                if len(T[ri, vehicle_idx]) < 3:
                    k = len(T[ri, vehicle_idx])
                else:
                    k = 3
                idx = np.argpartition(T[ri, vehicle_idx], k-1)
                # print(idx[:k])
                min_idx = list(idx[:k])
                # print(T[ri, vehicle_idx][idx[:k]])
                min_tt = list(T[ri, vehicle_idx][idx[:k]])
                # vid = vehicle_ids[min_idx]
                # print(vid)
                # vi = vehicle_idx[min_idx]
                # print(vi)
            else:
                min_idx.append(vi)
                min_tt.append(tt)
    ...
    def match(self, current_time, vehicles, requests):
        ...
        for coord in self.coord_iter():
            ...
            for i in range(int(np.ceil(len(R[coord]) / self.max_locations))):
                ...
                assignments = self.assign_nearest_vehicle(target_rids, candidate_vids, candidate_vidx, T.T)
                for vid, rid, tt in assignments:
                    match_list.append(self.create_matching_dict(vid, rid, tt))
    ...
    def match_RS(self, current_time, vehicles, requests):
        ...
        nV = n_vehicles*10        ######## Updated based on max init matching count
        # print(nV)
        if (n_vehicles > nV):
        ...
        T = self.eta_matrix(candidate_latlon, all_target_latlon)
        # print("match_D: ", dist, dist.shape)
        TRR = self.eta_matrix(all_target_latlon, all_target_latlon)
        TDD = self.eta_matrix(all_destination_latlon, all_destination_latlon)
        ...
        for coord in self.coord_iter():
            ...
            candidate_vidx = [all_candidate_vidx[v] for v in candidate_vids]
            # Call assign_nearest_k_vehicles instead
            assignments = self.assign_nearest_k_vehicle_RideShare(target_rids, all_target_ridx, requestAssigned,
                                                                candidate_vids,
                                                                candidate_vidx, T.T, TRR, TDD)
            for vid, rid, tt in assignments:
                commands.append(self.create_matching_dict(vid, rid, tt))
                # od_pairs.append((v_latlon.loc[vid], target_latlon.loc[rid]))
                ...
                #   V[vid2coord[vid]].remove(vid)
                if vehicle.state.init_match_count >= vehicle.state.max_init_matching and vid in V[vid2coord[vid]]:
                    V[vid2coord[vid]].remove(vid)
