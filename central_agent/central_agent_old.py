...
class Central_Agent(object):
        ...
        if len(requests) > 0:
            ...
            while(num_unmatched/total_reqs > 0.1):
                ...
                #     print(dfs)
                    ...
                    narrow_list, finalie, final_dict = self.greedy_wrapper(chunks)
                    e4 = t.time()
                    # p.close()
                    ...
                    # print("There")
                    narrow_list, finalie, final_dict = self.greedy_insertion(final_commands)
    ...
    def greedy_insertion(self, commands):
        # from simulator.models.vehicle.vehicle_repository import VehicleRepository
        # global VehicleRepository
        # print("Here")
        print(multiprocessing.current_process(), getpid())
        s = t.time()
    ...
    def greedy_wrapper(self, chunks):
        numCpu = multiprocessing.cpu_count()
        ...
        with multiprocessing.Pool(processes=numCpu) as p:
            narrow, commands, final_dict = zip(*p.imap(self.greedy_insertion, chunks, size))
        p.close()
        # p.join()
        p.terminate()
