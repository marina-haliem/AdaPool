...
class DQNDispatchPolicy(DispatchPolicy):
    ...
    def build_q_network(self, num, load_network=None):
        self.q_network = DeepQNetwork(num, load_network)
    ...
    # Overriding the parent function in dummy_agent.dispatch_policy
    def get_dispatch_decisions(self, tbd_vehicles, agent_logger, model_num):
        dispatch_commands = []
...
class DQNDispatchPolicyLearner(DQNDispatchPolicy):
    ...
    # Store memory
    def dump_experience_memory(self, num):
        sd_path = os.path.join(FLAGS.save_memory_dir, "sd_history_"+str(num)+".pkl")
        sars_path = os.path.join(FLAGS.save_memory_dir, "sars_history_"+str(num)+".pkl")
        pickle.dump(self.supply_demand_history, open(sd_path, "wb"))
        pickle.dump(self.experience_memory, open(sars_path, "wb"))

    # Load stored memory
    def load_experience_memory(self, path, num):
        sd_path = os.path.join(path, "sd_history_"+str(num)+".pkl")
        sars_path = os.path.join(path, "sars_history_"+str(num)+".pkl")
    ...
    def build_q_network(self, num, load_network=None):
        self.q_network = FittingDeepQNetwork(num, load_network)
    ...
    def get_dispatch_decisions(self, tbd_vehicles, agent_logger, model_num):
        dispatch_commands = []
        # print("Inside Rewards: ", self.rewards)
        for vehicle_id, vehicle_state in tbd_vehicles.iterrows():
            r = self.rewards[vehicle_id]
            if math.isinf(r) and r > 0:
                r = 10000000000
            elif math.isinf(r) and r < 0:
                r = -10000000000
            # print(vehicle_id, "R: ", self.rewards[vehicle_id], r)
            # Get best action for this vehicle and whether it will be offduty or not
            a, offduty = self.predict_best_action(vehicle_id, vehicle_state)
            if offduty:
                command = self.create_dispatch_dict(vehicle_id, offduty=True)
            ...
            if FLAGS.adaptive:
                x, y = vehicle_state.lat, vehicle_state.lon
                # print(vehicle_id, "R: ", r)
                if "offduty" in command:
                    continue
                if "cache_key" in command:
                    ...
                    t_x, t_y = command["destination"]
                summary = "{:d}, {:f}, {:f}, {:f}, {:f}, {:f}".format(model_num, x, y, r, t_x, t_y)
                agent_logger.log_model_event(summary)
                # print("Logging Model")
        return dispatch_commands
    ...
    def give_rewards(self, vehicles):
        for vehicle_id, row in vehicles.iterrows():
            ...
            self.rewards[vehicle_id] += (12 * earnings) - 5 * row.pickup_time + 10 * settings.STATE_REWARD_TABLE[
                row.status]
    ...
    def dispatch(self, current_time, vehicles, logger, num):
        self.give_rewards(vehicles)
        # print("Rewards: ", self.rewards)
        dispatch_commands = super().dispatch(current_time, vehicles, logger, num)
        self.backup_supply_demand()
        # If size exceeded, run training
        if len(self.supply_demand_history) > settings.INITIAL_MEMORY_SIZE:
            average_loss, average_q_max = self.train_network(FLAGS.batch_size, num)
    ...
    def train_network(self, batch_size, num, n_iterations=1):
        loss_sum = 0
        q_max_sum = 0
        ...
        self.q_network.run_cyclic_updates(num)     # Update target network
